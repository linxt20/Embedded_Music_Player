...
struct WAV_HEADER
{
    ...
    uint16_t audio_format;	  // 编码格式代码				 常见的WAV文件使用PCM脉冲编码调制格式，该数值通常为1。
    uint16_t num_channels;	  // 声道数                     单声道为1，立体声或双声道为2。
    uint32_t sample_rate;	  // 采样频率                   每个声道单位时间采样次数。常用的采样频率有11025，22050和44100kHz。
    ...
    uint16_t bits_per_sample; // 采样位数                   存储每个采样值所用的二进制数位数。常见的位数有4、8、12、16、24、32。

    char sub_chunk2_id[4];	  // 数据  不知道什么数据
    uint32_t sub_chunk2_size; // 数据大小
} wav_header;
static int wav_header_size; // 接收wav_header数据结构体的大小
...
static snd_pcm_uframes_t period_size = 2205 * 4;
